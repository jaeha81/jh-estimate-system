...
class Orchestrator:
    ...
    def run_pipeline(
        ...
        PDF 파일은 pdf_analyzer로 항목을 추출한 뒤 분류 파이프라인에 합류.
        Excel/xls 파일은 기존 template_mgr → normalizer 경로 유지.
        """
        db = get_db()
        ...
        try:
            ...
            if is_pdf:
                # ── PDF 경로 ──────────────────────────────────────────────
                # 2p. PDF 항목 추출 (오류 시 빈 리스트, 파이프라인 계속)
                pdf_items = self.pdf_analyzer.analyze(file_bytes, file_name)
                ...
                rows = [
                    {
                        ...
                        "unit_price": item.get("estimated_cost"),
                        "amount": None,
                    }
                    for item in pdf_items
                ]
                if rows:
                    db.table("estimate_line_items").insert(rows).execute()

                temp_path = None  # PDF는 임시 파일 불필요
            ...
            # 9b. Inspector 검수
            inspection = self.inspector.run(classified)
            if not inspection["passed"]:
                logger.info(
                    f"[inspector] 이슈 {len(inspection['issues'])}건 감지 "
                    f"— review_flag 유지"
                )
